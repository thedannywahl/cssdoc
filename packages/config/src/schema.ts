  ...
  properties: {
    ...
    tagDefinitions: {
      ...
      items: {
        ...
        properties: {
          ...
          recordKind: {
            enum: ["component", "utility", "rule", "declaration", "layout"],
          },
    ...
    rules: {
      ...
      properties: {
        ...
        "disallowed-element": { enum: ["off", "warn", "error"] },
        "unknown-state": { enum: ["off", "warn", "error"] },
        "unknown-part": { enum: ["off", "warn", "error"] },
        "undocumented-part": { enum: ["off", "warn", "error"] },
        "undocumented-css-part": { enum: ["off", "warn", "error"] },
        "component-name-case": { enum: ["off", "warn", "error"] },
        "part-name-case": { enum: ["off", "warn", "error"] },
        "duplicate-record-id": { enum: ["off", "warn", "error"] },
        "duplicate-record-id-cross-kind": { enum: ["off", "warn", "error"] },
        "structure-unknown-selector": { enum: ["off", "warn", "error"] },
        "structure-unknown-record": { enum: ["off", "warn", "error"] },
        "structure-ambiguous-record": { enum: ["off", "warn", "error"] },
        "invalid-default-value": { enum: ["off", "warn", "error"] },
        "invalid-property-value": { enum: ["off", "warn", "error"] },
        "invalid-fallback-value": { enum: ["off", "warn", "error"] },
        "unknown-custom-property": { enum: ["off", "warn", "error"] },
        "unknown-annotation-ref": { enum: ["off", "warn", "error"] },
    ...
    overrides: {
      description: "Per-glob rule severity overrides. Globs are relative to this config file.",
      type: "array",
      items: {
        type: "object",
        additionalProperties: false,
        ...
        properties: {
          files: {
            description: "One or more glob patterns relative to this config file.",
            oneOf: [{ type: "string" }, { type: "array", items: { type: "string" } }],
          },
          rules: {
            description: "Per-rule severity overrides for matching files (off/warn/error).",
            type: "object",
            additionalProperties: false,
            properties: {
              "missing-summary": { enum: ["off", "warn", "error"] },
    ...
    ruleOptions: {
      description: "Policy options for rule behavior families.",
      type: "object",
      additionalProperties: false,
      properties: {
        values: {
          ...
          properties: {
            allowInherit: {
              type: "boolean",
              description: "Allow `inherit` in sealed/frozen value checks (default false).",
            },
          },
        },
        sealed: {
          ...
          properties: {
            mode: {
              enum: ["strict", "compat"],
              description: "strict flags full reset-keyword set; compat limits to unset/initial.",
            },
          },
        },
      },
    ...
    render: {
      ...
      properties: {
        ...
        structureVariantView: {
          description:
            'How alternative DOM shapes (from top-level @variant blocks in @structure) render: "diagram" (default) emits one combined flowchart with a labelled subgraph per variant; "sections" emits a "### Variant: <name>" subsection per variant.',
          enum: ["diagram", "sections"],
        },
        groups: {
          description:
